...
const cardSchema = z.object({
  ...
  expiry: z.string().regex(/^(0[1-9]|1[0-2])\/([0-9]{2})$/, 'Expiry date must be in MM/YY format'),
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 container py-12 text-center">
          ...
          <p className="mb-6">There was a problem with your payment information. Please return to your cart.</p>
          <Button 
  ...
  const onBankSubmit = async (values: z.infer<typeof bankTransferSchema>) => {
    ...
    try {
      ...
      toast("Process Failed", {
        description: "There was a problem initiating your bank transfer. Please try again."
      });
    } finally {
      setIsProcessing(false);
    }
              ...
              <CardContent>
                <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-4 mb-6">
                  ...
                  <div className="flex items-center space-x-2 border p-4 rounded-md">
                    ...
                    <Label htmlFor="bank" className="flex-1">Bank Transfer</Label>
                  </div>
                </RadioGroup>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span>${(paymentData.amount - paymentData.taxAmount - paymentData.deliveryFee).toFixed(2)}</span>
                  </div>
                  
                  {paymentData.deliveryFee > 0 && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Delivery fee ({paymentData.deliveryArea})</span>
